<div style="margin: 50px">
    <div class="form-row" style="margin-top: -20px">
        <div class="form-group col-md-3" style="margin-top: 1px">
            ...
            <select class="form-control" wire:model="accommodatieFilter">
                <option value="0">-</option>
                @foreach ($accommodatieslijst as $accommodatielijst)
                <option value="{{ $accommodatielijst->naam }}">{{ $accommodatielijst->naam }}</option>
                @endforeach
            </select>
        </div>
    ...
    <div class="card border-secondary" style="margin-top: 10px; height: 250px;">
        <div class="row">
            <div class="col">
                <img class="rounded border border-dark card-img" src='{{ URL::asset("images/accommodatie/$bestemming->naam.jpg") }}' style="height: 200px; width: 350px; margin: 25px">
            </div>
            <div class="col-xl-8 mt-4">
                <a style="text-decoration: none" href="/accommodatie/bekijken/{{ $bestemming->idbestemming }}">
                    <h4 style="margin-bottom: 0px" class="text-dark fw-bold">{{ $bestemming->naam }}</h4>
                </a>
                ...
                <a class="text-dark text-decoration-none" style="text-decoration: none; " href="/accommodatie/bekijken/{{ $bestemming->idbestemming }}">
                ...
                <p class="card-text mt-3" style="margin-bottom: -10px; font-size: 14px">{{ $bestemming->plaats }}, {{ $bestemming->land->land }}</p>
            </div>
        </div>
        </a>
        @endforeach
